...
def writer():
    return Agent(
        ...
        llm=get_llm_for_role("writer", llm_type="openai"),
        verbose=True,
...
def director():
    return Agent(
        ...
        llm=get_llm_for_role("director", llm_type="openai"),
        verbose=True,
    )
...
def checker():
    return Agent(
        ...
        你只负责报告问题，不负责修改。""",
        llm=get_llm_for_role("checker", llm_type="openai"),
        verbose=True,
    )
...
def curator():
    return Agent(
        ...
        llm=get_llm_for_role("curator", llm_type="openai"),
        verbose=True,
    )
...
def polisher():
    return Agent(
        role="文学润色师",
        goal="识别和去除 AI 生成文本的痕迹，使文字听起来更自然、更有人味。",
        ...
        你必须输出纯正的【简体中文】。""",
        llm=get_llm_for_role("polisher", llm_type="openai"),
        verbose=True,
    )
